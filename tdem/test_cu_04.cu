...
try
{
    ...
    if (argc < 2)
        throw new Fatal("This program must be called with one argument: the name of the data input file without the '.inp' suffix.\nExample:\t %s filekey\n", argv[0]);
    // number of threads
    size_t Nproc = 1;
    double dthoot = 4.0;
    double qin = 0.0;
    ...
    size_t modeContact = 0;
    if (argc >= 6)
        modeContact = atoi(argv[5]);

    if (argc >= 7)
        DEM::delta_cutoff = atof(argv[6]);

    if (modeContact == 0)
    {
        cout << "--------------Using linear contact model--------------\n";
        cout << "-------------- delta_cutoff is set to be " << DEM::delta_cutoff << " ----------\n";
    ...
    {
        cout << "--------------Using hertz contact model--------------\n";
        cout << "-------------- delta_cutoff is set to be " << DEM::delta_cutoff << " ----------\n";
    ...
    {
        if (ny == 1)
            dom.AddVoroPack(-1, R, Lx, Ly, Lz, nx, ny, nz, rho, Cohesion, bVec3_t(true, false, true), seed, fraction, Vec3_t(qin, 1.0, qin));
        else
            dom.AddVoroPack(-1, R, Lx, Ly, Lz, nx, ny, nz, rho, Cohesion, bVec3_t(true, true, true), seed, fraction, Vec3_t(qin, qin, qin));
    }
    else if (ptype == "tetra")
    {
        ...
        dom.GenRice(-1, Lx, nx, R, rho, seed, fraction);
    else
    {
        dom.Load(ptype.CStr());
        load = true;
