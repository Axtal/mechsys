...
namespace DEM
{
    ...
    __device__ bool CalcForceVV_UseLinearContact(InteractonCU const *Int,
                                                 ComInteractonCU *CInt,
                                                 DynInteractonCU *DIntVV,
                                                 ParticleCU *Par1,
                                                 ParticleCU *Par2,
                                                 ...
                                                 real3 *T2)
    {
        ...
        real3 Branch;
        if ((*Int).BothFree)
            BranchVec(xf, xi, Branch, demaux[0].Per);
        else
            Branch = xi - xf;
        ...
        (*DIntVV).Fn = make_real3(0.0, 0.0, 0.0);
        if (delta > 0.0)
        {
            ...
            // Linear -------------------
            (*DIntVV).Fn = (*Int).Kn * delta * n;
            (*DIntVV).Ft = (*DIntVV).Ft + ((*Int).Kt * demaux[0].dt) * vt;
            ...
            (*DIntVV).Fr = (*DIntVV).Fr + ((*Int).Beta * ((*Int).Kt) * demaux[0].dt) * vr;
            ...
            (*DIntVV).F = (*DIntVV).Fn + (*DIntVV).Ft + (*Int).Gn * dotreal3(n, vrel) * n + (*Int).Gt * vt;
    ...
    __device__ bool CalcForceVV_UseHertzContact(InteractonCU const *Int,
                                                ComInteractonCU *CInt,
                                                DynInteractonCU *DIntVV,
                                                ParticleCU *Par1,
                                                ParticleCU *Par2,
    ...
    //----------------------Pointer ---------------------------------
    //----------------------Pointer ---------------------------------
    __device__ Device_CalcForceVV_ContactPointer devFuncPtrLinearContact = CalcForceVV_UseLinearContact;
    __device__ Device_CalcForceVV_ContactPointer devFuncPtrHertzContact = CalcForceVV_UseHertzContact;
    Device_CalcForceVV_ContactPointer deviceFunction_inside_CalcForceVV;
    //--------------------------------------------------------
    ...
    __global__ void CalcForceVV(InteractonCU const *Int, ComInteractonCU *CInt, DynInteractonCU *DIntVV, ParticleCU *Par, DynParticleCU *DPar, dem_aux const *demaux,
                                Device_CalcForceVV_ContactPointer f)
    {
        size_t ic = threadIdx.x + blockIdx.x * blockDim.x;
        if (ic >= demaux[0].nvvint)
            return;
        ...
        real3 T1, T2;
        bool If_delta_larger_than_zero =
            f(&Int[id], &CInt[id], &DIntVV[ic], &Par[i1], &Par[i2], &DPar[i1], &DPar[i2], demaux, &T1, &T2);

        // InteractonCU const *Int,
        // ComInteractonCU *CInt,
        // DynInteractonCU *DIntVV,
        // ParticleCU *Par1,
        // ParticleCU *Par2,
    ...
    __global__ void CalcForceEE(size_t const *Edges, real3 const *Verts, InteractonCU const *Int, ComInteractonCU *CInt, DynInteractonCU *DIntEE, ParticleCU *Par, DynParticleCU *DPar, dem_aux const *demaux)
    {
        ...
        if (Int[id].BothFree)
            Pert = demaux[0].Per;
        if (!OverlapEE(Edges, Verts, f1, f2, dm1, dm2, Pert))
            return;
    ...
    __global__ void CalcForceVF(size_t const *Faces, size_t const *Facid, real3 const *Verts, InteractonCU const *Int, ComInteractonCU *CInt, DynInteractonCU *DIntVF, ParticleCU *Par, DynParticleCU *DPar, dem_aux const *demaux)
    {
        ...
        if (Int[id].BothFree)
            Pert = demaux[0].Per;
        if (!OverlapVF(Faces, Facid, Verts, xi, f2, dm1, dm2, Pert))
            return;
    ...
    __global__ void CalcForceFV(size_t const *Faces, size_t const *Facid, real3 const *Verts, InteractonCU const *Int, ComInteractonCU *CInt, DynInteractonCU *DIntFV, ParticleCU *Par, DynParticleCU *DPar, dem_aux const *demaux)
    {
        ...
        if (Int[id].BothFree)
            Pert = demaux[0].Per;
        if (!OverlapFV(Faces, Facid, Verts, f1, xf, dm1, dm2, Pert))
            return;
    ...
    __global__ void ResetMaxD(real3 *Verts, real3 *Vertso, real *maxd, ParticleCU const *Par, DynParticleCU *DPar, dem_aux const *demaux)
    {
        ...
        if (isfree)
        {
            if (demaux[0].px)
            {
                if (DPar[ic].x.x < demaux[0].Xmin)
                    dis.x = demaux[0].Xmax - demaux[0].Xmin;
                if (DPar[ic].x.x >= demaux[0].Xmax)
                    dis.x = demaux[0].Xmin - demaux[0].Xmax;
            }
            if (demaux[0].py)
            {
                if (DPar[ic].x.y < demaux[0].Ymin)
                    dis.y = demaux[0].Ymax - demaux[0].Ymin;
                if (DPar[ic].x.y >= demaux[0].Ymax)
                    dis.y = demaux[0].Ymin - demaux[0].Ymax;
            }
            if (demaux[0].pz)
            {
