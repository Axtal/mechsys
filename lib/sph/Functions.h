...
inline double indpAA(Mat3_t const & A){
	// Inner double product of a symmetric Matrix A(3x3) -> A^2=A:A=tr(A.A')=AijAij
    return (A(0,0)*A(0,0) + A(1,1)*A(1,1) + A(2,2)*A(2,2) + 2.0*(A(0,1)*A(0,1) + A(0,2)*A(0,2) + A(1,2)*A(1,2)));
}

inline double indpAB(Mat3_t const & A, Mat3_t const & B){
	// The double contraction of two matrices A(3x3) and B(3x3) -> AB=A:B=tr(A.B')=AijBij
    return (A(0,0)*B(0,0) + A(1,1)*B(1,1) + A(2,2)*B(2,2) + 2.0*(A(0,1)*B(0,1) + A(0,2)*B(0,2) + A(1,2)*B(1,2))); 
}
